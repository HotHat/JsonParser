...
public class JsonParse {
	...
	private JsonValue parseJsonValue() throws JsonParseException {
		...
		if (tk.content.equals("true")) {
			...
			value.setType(JsonTypes.TRUE);
			value.setSpecial("true");
			tokenIndexForward();
			
		} else if (tk.content.equals("false")) {
			value.setType(JsonTypes.FALSE);
			value.setSpecial("false");
			tokenIndexForward();
			
		} else if (tk.content.equals("null")) {
			value.setType(JsonTypes.NULL);
			value.setSpecial("null");
			tokenIndexForward();
			
		} else if (tk.content.equals("{")) {
			value.setType(JsonTypes.OBJECT);
			value.setObj(parseObject());
			
			
		} else if (tk.content.equals("[")) {
			value.setType(JsonTypes.ARRAY);
			value.setArray(parseJsonArray());
			
		} else if (tk.type == JsonTypes.NUMBER) {
			
			value.setType(JsonTypes.NUMBER);
			value.setNumber(Double.valueOf(tk.content));
			tokenIndexForward();
			
		} else if (tk.type == JsonTypes.STRING) {
			
			value.setType(JsonTypes.STRING);
			value.setString(tk.content);
			tokenIndexForward();
	...
	private JsonArray parseJsonArray() throws JsonParseException {
		ArrayList<JsonValue> jes = new ArrayList<>();
	...
	private JsonObject parseObject() throws JsonParseException {
		...
		if (tk.content.equals("{")) {
			...
			JsonMembers jMembers = parseJsonMembers();
			
			Token tkend = getToken();
			tokenIndexForward();
			
			if (tkend.content.equals("}")) {
				return new JsonObject(jMembers);
			} else {
